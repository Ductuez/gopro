...
async function main() {
  ...
  matches.forEach((match) => {
    ...
    if (!grouped[lid])
      grouped[lid] = {
        league: match.league || { id: lid, name: "Unknown" },
        matches: [],
  ...
  await client.set(
    key,
    JSON.stringify({ data: groupedArr, fetchedAt: Date.now() }),
    { PX: ttl },
  );
  console.log(
    `Wrote ${groupedArr.length} league blocks to Redis key ${key} with TTL ${ttl}ms`,
  );
