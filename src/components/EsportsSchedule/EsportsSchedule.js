...
export default function EsportsSchedule() {
  ...
  const matchesTodayFromStore = useSelector(
    (state) => state.matchesToday.data || [],
  );
  ...
  const fetchMatches = async (date = selectedDate) => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/matches?begin_at=${encodeURIComponent(begin)}&end_at=${encodeURIComponent(end)}`,
      );
    ...
    <div className="text-white rounded-xl p-6 w-full shadow-2xl border bg-black-700 border-white-10 bg-opacity-80">
      ...
      {cacheBadge && (
        <div className="mb-3 text-xs text-gray-300">
          Cache: <span className="font-medium">{cacheBadge}</span>
        </div>
      )}
