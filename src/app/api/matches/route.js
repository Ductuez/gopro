...
export async function GET(req) {
  ...
  const forceMock = searchParams.get('mock') === '1' || searchParams.get('fixture') === '1'
  ...
  const paramsSchema = z.object({
    begin_at: z.string().refine((s) => !Number.isNaN(Date.parse(s)), { message: "begin_at must be valid ISO datetime" }),
    end_at: z.string().refine((s) => !Number.isNaN(Date.parse(s)), { message: "end_at must be valid ISO datetime" }),
    per_page: z.number().int().min(1).max(100).optional(),
  })

  const parsed = paramsSchema.safeParse({ begin_at: defaultBegin, end_at: defaultEnd, per_page: perPage })
  if (!parsed.success) {
  ...
  if (Date.parse(defaultBegin) > Date.parse(defaultEnd)) {
    return NextResponse.json({ error: "begin_at must be before or equal to end_at" }, { status: 400 })
  }
  ...
  return NextResponse.json(normalizeGrouped(grouped))
      ...
      return NextResponse.json({ error: "fixture load failed" }, { status: 500 })
    }
  }
  ...
  const cached = await getCached(cacheKey)
    ...
    if (cached) {
      ...
      if (age < DEFAULT_TTL + STALE_WHILE_REVALIDATE) {
        ...
        return new Response(JSON.stringify(cached.data), { headers: { 'content-type': 'application/json', 'x-cache': 'STALE' } })
      }
    }

  const grouped = await fetchAndAggregateMatches({ begin: defaultBegin, end: defaultEnd, perPage, maxPages: DEFAULT_MAX_PAGES, token: TOKEN_PANDASCORE })
...
function normalizeMatch(m) {
  ...
  if (!winner_id && Array.isArray(games)) {
    const lastFinished = [...games].reverse().find((g) => g?.status === 'finished' && g?.winner?.id)
    if (lastFinished) winner_id = lastFinished.winner.id
  }

  const league = m.league || { id: m.league_id ?? 'unknown', name: m.league_name ?? 'Unknown', image_url: m.league_image_url ?? null }

  function countWinsForTeam(teamId) {
    return games?.filter((g) => g?.status === 'finished' && g?.winner?.id === teamId).length || 0
  }
  ...
  const is_live = (m.status === 'running' || m.status === 'live' || m.is_live === true)
  const ui_status = is_live ? 'live' : isMatchFinished ? 'finished' : 'upcoming'
  ...
  const stream_url = m.stream_url ?? (m.streams && m.streams[0] && m.streams[0].url) ?? null
...
async function setCached(key, value) {
  if (redisClient) {
    try {
      await redisClient.set(key, JSON.stringify(value), { PX: DEFAULT_TTL + STALE_WHILE_REVALIDATE })
      return
    } catch (e) {
