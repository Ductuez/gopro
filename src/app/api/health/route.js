...
export async function GET() {
  ...
  if (!pandascoreKey) {
    status.ok = false
    status.details.pandascore = { ok: false, reason: 'PANDASCORE_API_KEY missing' }
  } else {
    try {
      const res = await fetch('https://api.pandascore.co/lol/matches?per_page=1', {
  ...
  if (REDIS_URL) {
    ...
    status.details.redis = { ok: false, reason: 'REDIS_URL not set (optional)' }
  }
